function [pks_locs_Data] = plotSelectChannelsPoints(EKGON,SpO2ON,PArtON,CapnoON, FlowON,PActON,PPlON, PAbON,VolAutoON,VolAbsON,EKGData,SpO2Data,PArtData,CapnoData,FlowData,PActData,PPlData,PAbData,VolAutoData,VolAbsData, time, Title)
...
title(Title)
    for n = 1:S 
        ydata = refData(ON(n),:);
        subplot(S,1,n)
        [pks,locs]=findpeaks(ydata,xdata,'MinPeakDistance',1); %initial 0.001
        plot(locs,pks,'.');
        ylabel(Labels{ON(n)});
        xlim([time(1) time(end)])
        pks_locs_Data{n,1} = pks;
        pks_locs_Data{n,2} = locs;

        switch Labels{ON(n)} 
            case 'Actuator Pressure [psi]'
                ylabel({'Peak Actuator';'Pressure [psi]'})
            case 'Flow [L/s]'
%                 ylim([-0.5 0.4])
