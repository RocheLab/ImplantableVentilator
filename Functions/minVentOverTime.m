function [TminVent,PminVent,NminVent] = minVentOverTime(FlowData,timeStart,timeDur,dt,plotON,smoothON, shiftON,calPlotON, zeroshift,scale)
...
%   timeStart = the starting point of the selection in the data (in mseconds)
%   timeDur = the length of data selected (in mseconds)
...
Ndt = ceil(timeDur/dt);
overTime = dt*Ndt;
timerange = [(timeStart*1000)+1:((timeStart+overTime)*1000)];%set timerange to slightly larger than selection to ensure there are enough data points for the array based on specified dt
data = FlowData(timerange);
...
dataIntN = reshape(neg,[dt*1000,Ndt]);%data reshaped into an array to perform a cumulative trapezoidal integration over
