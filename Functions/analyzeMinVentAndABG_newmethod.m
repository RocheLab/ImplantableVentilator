function analyzeMinVentAndABG_newmethod(date,time,newVolAutoData, T,ABGTimes,pHvalues,pCO2values,pO2values,sO2values, baseline, SDbaseline,PActData,DateTime,plotsON) 
...
for i = 1:length(ABGTimes)
    ...
    DTtime = DateTime+seconds(time);
    timeindlogical = find(DTtime >= ABGTimes(i)); 
    closestInd = timeindlogical(1);
    PeriodStartIndLogical = find(DTtime >= ABGTimes(i)-seconds(T)); 
    PeriodStartInd = PeriodStartIndLogical(1);
    indices = (PeriodStartInd:closestInd);
%     AvgMinVent = mean(PminVent(indices));
[Vpks,Vlocs]=findpeaks(newVolAutoData(indices),time(indices),'MinPeakDistance',0.6,'MinPeakProminence',0.01);%%tidal volumes
%  findpeaks(newVolAutoData(indices),time(indices),'MinPeakDistance',1.4,'MinPeakHeight',0.008,'MinPeakProminence',0.01);%%tidal volumes

    totvent = sum(Vpks(2:end)); %sum 1 fewer total vol to account for the time calculated for tottime
    tottime = Vlocs(end)-Vlocs(1);
    AvgMinVent = totvent*60/(tottime); %L/min
