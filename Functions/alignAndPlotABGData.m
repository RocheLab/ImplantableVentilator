function [ABGTimes,pHvalues,pCO2values,pO2values,sO2values] = alignAndPlotABGData(date,DateTime, timeStart, timeDur, time,PArtData)
...
logicalStart = find(ABGDataCompiled{:, 1} >= (selectionStart-minutes(3))); %gives a 3 minute buffer just in case
%find the last ABG in range
logicalEnd = find(ABGDataCompiled{:, 1} <= (selectionEnd+minutes(3))); %gives a 3 minute buffer just in case
...
%ABG data in range
ABGTimes = ABGDataCompiled{startInd:endInd, 1}%column 1 has slightly better alignment with powerlab times
    ...
    response = input(prompt,'s');
        ...
        elseif response == 'N'
            ...
            if response2 == 'Y'
                ...
                ABGTimes = ABGDataCompiled{indices, 1};%column 1 has slightly better alignment with powerlab times
...
elseif NPArt < NPaper
    disp('A mismatch is found within the number of ABGs in this section.')
    fprintf('Within this time range +/-3 minutes, there are %d values found from the paper times and %d values found from the PArt waveform.\n',NPaper,NPArt)
