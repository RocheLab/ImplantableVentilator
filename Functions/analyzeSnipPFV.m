function [Output] = analyzeSnipPFV(FlowData,VolAutoData,Vollocs,PActlocs,Flowlocs,Vminbounds,PActbounds)
...
Output = zeros(1,17); % content is [bound1 bound2 Fpkloc Fpkval Ppkloc Vpkloc Vpkval F_P P_V F_V];
%%% update 7/24: Output content is [bound1 bound2 Fpkloc Fpkval Ppkloc Pminloc Vpkloc Vpkval Vminloc F_P P_V F_V F_Pmin Pmin_V Pmin_Vmin  P_Vmin F_Vmin ];
...
while bound1 < boundlookup(end) %may need to change this as the for loop bounds
    ...
    if isempty(Vol_i) && isempty(PAct_i) && isempty(Flow_i)
        ...
        bound1 = bound2;
        if isempty(nextbound)
            bound2 = bound2 + 10000;
        else
            bound2 = boundlookup(nextbound(1));
        end
        continue
    elseif length(Vol_i) == 1 && length(PAct_i) == 1 && length(Flow_i) == 1
        ...
        %%%% added new metrics 7/24        
        listVmin = find(Vminbounds<Vpkloc);
        Vminloc = Vminbounds(listVmin(end));
        listPmin = find(PActbounds<Ppkloc); % find the clossest PActmin (start of an actuation wave) before the Ppkloc
        ...
        if isempty(listPmin)
            ...
            Pmin_Vmin = NaN; 
            P_Vmin = NaN; 
            F_Vmin = Fpkloc - Vminloc; % pos if Vol start is first, neg if Flow peak is first  

        else
            Pminloc = PActbounds(listPmin(end));
            
            F_Pmin = Fpkloc - Pminloc; % pos if PAct start is first, neg if Flow peak is first
            Pmin_V = Pminloc - Vpkloc; % pos if Vol peak is first, neg if PAct start is first
            Pmin_Vmin = Pminloc - Vminloc; % pos if Vol start is first neg if PAct start is first
            P_Vmin = Ppkloc - Vminloc; % pos if Vol start is first, neg if Ppk is first
            F_Vmin = Fpkloc - Vminloc; % pos if Vol start is first, neg if Flow peak is first  
       end 
        ...
        Output(row,:) = [bound1 bound2 Fpkloc Fpkval Ppkloc Pminloc Vpkloc Vpkval Vminloc F_P P_V F_V F_Pmin Pmin_V Pmin_Vmin  P_Vmin F_Vmin ];
    ...
    elseif length(Vol_i) == 1 && (length(Flow_i)>1 || length(PAct_i)>1)% either PAct of F act has more than one but only 1 vol pk
        ...
        if isempty(PAct_i)
            ...
            Pminloc = 0;
            %%%% added new metrics 7/24  
            F_Pmin = NaN; % pos if PAct start is first, neg if Flow peak is first
            Pmin_V = NaN; % pos if Vol peak is first, neg if PAct start is first
            Pmin_Vmin = NaN; % pos if Vol start is first neg if PAct start is first
            P_Vmin = NaN; % pos if Vol start is first, neg if Ppk is first
            F_Vmin = NaN; % pos if Vol start is first, neg if Flow peak is first   
            %%%%
        elseif length(PAct_i) == 1
            ...
            [Ppk,Pind] = min(abs(PActlocs(PAct_i)-Vpkloc));
